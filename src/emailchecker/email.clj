...
(defn socket-error-message
  [error-code]
  (case error-code
    :missing-host "No mx host"
    :timeout "mx connect timeout"))
...
(defn check
  ...
  (if (valid-format? email)
    (let [host (get-mx (get-domain email))
          ...
          socket-open (if-not (nil? host)
                        (try @(client host port :ssl? false)
                          (catch Exception e :timeout))
                        :missing-host)
          socket (if-not (keyword? socket-open)
                   socket-open)]
      (if-not (nil? socket)
        (let [f1  (get-next-response socket timeout)
          ...
          validity)
        {:error? true
         :message (socket-error-message socket-open)}))
    {:error? true
